...
namespace ImGui
{
	...
	IMGUI_API bool          BeginAndroid(const char* name, bool* p_open = NULL, ImGuiWindowFlags flags = 0);   // push window to the stack and start appending to it. see .cpp for details. return false when window is collapsed, so you can early out in your code. 'bool* p_open' creates a widget on the upper-right to close the window (which sets your bool to false).
	...
	IMGUI_API bool          MyButton(const char* label, bool inuse, const ImVec2& size_arg);    // button
	IMGUI_API bool          AndroidButton(const char* label, const ImVec2& size = ImVec2(0, 0));            // button
...
enum ImGuiWindowFlags_
{
	...
	ImGuiWindowFlags_ChildZeusPaste = 1 << 18,
    // [Internal]
...
struct ImDrawList
{
	...
	IMGUI_API void AddHealthBar(const ImVec2& pos, ImU32 col, float thickness, const int size);
