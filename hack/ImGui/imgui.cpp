...
static bool BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
	...
	if (extra_flags == ImGuiWindowFlags_ChildZeusPaste)
	{
		...
		parent_window->DrawList->AddLine(ImVec2(child_bb.x, child_bb.y), ImVec2(child_bb.x, child_bb.y + size_arg.y), ImColor(100, 100, 100));
		parent_window->DrawList->AddLine(ImVec2(child_bb.x + size_arg.x, child_bb.y), ImVec2(child_bb.x + size_arg.x, child_bb.y + size_arg.y), ImColor(100, 100, 100));
		parent_window->DrawList->AddLine(ImVec2(child_bb.x, child_bb.y + size_arg.y), ImVec2(child_bb.x + size_arg.x, child_bb.y + size_arg.y), ImColor(100, 100, 100));
		parent_window->DrawList->AddLine(ImVec2(child_bb.x, child_bb.y), ImVec2(child_bb.x + 10, child_bb.y), ImColor(100, 100, 100));
		parent_window->DrawList->AddLine(ImVec2(child_bb.x + 20 + value_bufsize.x, child_bb.y), ImVec2(child_bb.x + size_arg.x, child_bb.y), ImColor(100, 100, 100));
		parent_window->DrawList->AddText(ImVec2(child_bb.x + 15,child_bb.y - 4), ImColor(150, 150,150), name);
...
bool ImGui::BeginAndroid(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
	ImGuiContext& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	IM_ASSERT(name != NULL);                        // Window name required
	...
	if (first_begin_of_the_frame)
	{
		...
		else
		{
			...
			// Window background, Default Alpha
			window->DrawList->AddRectFilled(window->Pos, window->Pos + window->Size, GetColorU32(ImGuiCol_WindowBg), window_rounding);
			window->DrawList->AddRectFilled(window->Pos + ImVec2(82, 0), window->Pos + window->Size, GetColorU32(ImGuiCol_Border), window_rounding);

			window->DrawList->AddRectFilled(window->Pos + ImVec2(82, 0), window->Pos + ImVec2(83, 14), GetColorU32(ImGuiCol_Separator), window_rounding);
			window->DrawList->AddRectFilled(window->Pos + ImVec2(82, 480), window->Pos + ImVec2(83, window->Size.y), GetColorU32(ImGuiCol_Separator), window_rounding);

			window->DrawList->AddRect(window->Pos - ImVec2(2, 6), window->Pos + ImVec2(window->Size.x + 2, window->Size.y + 6), ImColor(0.0f, 0.0f, 0.0f, 1.00f), window_rounding);
			window->DrawList->AddRect(window->Pos - ImVec2(1, 5), window->Pos + ImVec2(window->Size.x + 1, window->Size.y + 5), ImColor(0.1f, 0.1f, 0.1f, 1.00f), window_rounding);
			window->DrawList->AddRect(window->Pos - ImVec2(0, 4), window->Pos + ImVec2(window->Size.x, window->Size.y + 4), ImColor(0.15625f, 0.15625f, 0.15625f, 1.00f), window_rounding);
			window->DrawList->AddRect(window->Pos - ImVec2(-1, 3), window->Pos + ImVec2(window->Size.x - 1, window->Size.y + 3), ImColor(0.15625f, 0.15625f, 0.15625f, 1.00f), window_rounding);
			window->DrawList->AddRect(window->Pos - ImVec2(-2, 2), window->Pos + ImVec2(window->Size.x - 2, window->Size.y + 2), ImColor(0.15625f, 0.15625f, 0.15625f, 1.00f), window_rounding);
			window->DrawList->AddRect(window->Pos - ImVec2(-3, 1), window->Pos + ImVec2(window->Size.x - 3, window->Size.y + 1), ImColor(0.1f, 0.1f, 0.1f, 1.00f), window_rounding);
...
bool ImGui::MyButtonEx(const char* label, bool inuse, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	if (inuse)
	{
		RenderFrame(ImVec2(bb.Min.x - 35, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), GetColorU32(ImGuiCol_Border));
		RenderFrame(ImVec2(bb.Min.x - 25, bb.Min.y + 1), ImVec2(bb.Max.x, bb.Min.y), GetColorU32(ImGuiCol_Separator));
		RenderFrame(ImVec2(bb.Min.x - 25, bb.Max.y - 1), ImVec2(bb.Max.x, bb.Max.y), GetColorU32(ImGuiCol_Separator));
	}
	else if (!inuse)
		RenderFrame(ImVec2(bb.Max.x - 1, bb.Min.y - 4), ImVec2(bb.Max.x, bb.Max.y + 4), GetColorU32(ImGuiCol_Separator));
...
{
	return MyButtonEx(label, inuse, size_arg, 0);
}

bool ImGui::AndroidButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	*/
	...
	// Render
	const ImU32 col = GetColorU32((hovered && held) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
	RenderFrame(ImVec2(bb.Min.x +  3, bb.Min.y+3), ImVec2(bb.Max.x-3,bb.Max.y-28), col, true, style.FrameRounding);
	RenderFrame(ImVec2(bb.Min.x + 3, bb.Min.y + 9), ImVec2(bb.Max.x - 3, bb.Max.y - 22), col, true, style.FrameRounding);
	RenderFrame(ImVec2(bb.Min.x + 3, bb.Min.y + 15), ImVec2(bb.Max.x - 3, bb.Max.y - 16), col, true, style.FrameRounding);


	return pressed;
}

bool ImGui::AndroidButton(const char* label, const ImVec2& size_arg)
{
	return AndroidButtonEx(label, size_arg, 0);
}
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	RenderFrame(frame_bb.Min + ImVec2(0, 3), frame_bb.Max - ImVec2(2, 3), GetColorU32(ImGuiCol_FrameBg), true);
	...
	window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(0, 3), grab_bb.Max - ImVec2(0, 3), ImColor(116, 219, 51), ImColor(116, 219, 51), ImColor(78, 149, 33), ImColor(78, 149, 33));

	char value_buf[64];
	const char* value_buf_end = value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), "%.0f", *v);
	const ImVec2 value_bufsize = CalcTextSize(value_buf);
	RenderText(ImVec2(grab_bb.Max.x - value_bufsize.x/2, grab_bb.Max.y - 4), value_buf, value_buf_end, NULL);
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x , frame_bb.Min.y - 8.5), label);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	if (*v)
	{
		//window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max - ImVec2(24, 2), ImColor(0.30f, 0.30f, 0.30f));
		window->DrawList->AddRect(check_bb.Min, check_bb.Max - ImVec2(24, 2), ImColor(0.10f, 0.10f, 0.10f));
		window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(1,1), check_bb.Max - ImVec2(25, 3), ImColor(116, 219, 51),ImColor(116, 219, 51), ImColor(78, 149, 33), ImColor(78, 149, 33));
	}
	else
	{
		//window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max - ImVec2(24, 2), ImColor(0.40f, 0.40f, 0.40f));
		window->DrawList->AddRect(check_bb.Min, check_bb.Max - ImVec2(24, 2), ImColor(0.10f, 0.10f, 0.10f));
		window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(25, 3), ImColor(49, 51, 47), ImColor(49, 51, 47), ImColor(32, 33, 31), ImColor(32, 33, 31));
	}

	if (label_size.x > 0.0f)
		RenderText(text_bb.GetTL() - ImVec2(16,3), label);
	return pressed;
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(0.19f, 0.19f, 0.19f), true);

	const ImVec2 SizeText = CalcTextSize(preview_value);
   ...
   /* if (preview_value != NULL)*/
	...
	//if (label_size.x > 0)
	//{
		RenderFrame(ImVec2(frame_bb.Max.x - style.FramePadding.x - SizeText.x - 18, frame_bb.Min.y + style.FramePadding.y), ImVec2(frame_bb.Max.x - style.FramePadding.x - 14, frame_bb.Max.y - style.FramePadding.y), ImColor(0.25f, 0.25f, 0.25f), true,3.f);
		RenderText(ImVec2(frame_bb.Max.x - style.FramePadding.x - SizeText.x - 16, frame_bb.Min.y + style.FramePadding.y), preview_value);
	//}
