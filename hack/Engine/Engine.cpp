...
//[junk_disable /]
	...
	void ChatSpamCustom()
	{
		...
		float flServerTime = pPlayer->GetTickBase() * Interfaces::GlobalVars()->interval_per_tick;

		if (nextTime > flServerTime)
			return;

		nextTime = flServerTime + Settings::Misc::misc_ChatSpamDelay;

		if (!Settings::Misc::misc_CustomChatSpam)
			return;

		char str[512];
		sprintf(str, "say %s", Settings::Misc::misc_ChatSpamText);

		if (Interfaces::Engine()->IsConnected())
			Interfaces::Engine()->ClientCmd_Unrestricted2(str);
