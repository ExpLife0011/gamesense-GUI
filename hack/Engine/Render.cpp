...
namespace Engine
{
	...
	bool CRender::CreateObject()
	{
		...
		if (!m_pFont)
		{
			HRESULT hFontRes = D3DXCreateFontA(m_pDevice, 12, 0, FW_HEAVY, 0, FALSE, RUSSIAN_CHARSET, OUT_OUTLINE_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH | FW_DONTCARE, "Verdana", &m_pFont);
		...
		if (!m_pIconz)
		{
			HRESULT hFontRes = D3DXCreateFontA(m_pDevice, 12, 0, FW_HEAVY, 0, FALSE, DEFAULT_CHARSET, OUT_OUTLINE_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH | FW_DONTCARE, "AstriumWep", &m_pIconz);
	...
	{
		DWORD dxFillBoxColor = D3DCOLOR_RGBA(color.r(), color.g(), color.b(), int(Settings::Esp::esp_FillBoxOpacity));
